...
namespace AyzPaymentWizard
{
    public partial class Anasayfa : Form
    {
        ...
        private void Anasayfa_Load(object sender, EventArgs e)
        {
            ...
            dataGridViewPacket.Columns["ApprovalNote"].Width = 200;
            ...
            dataGridViewPacket.Columns["StatusMask"].Width = 200;
            ...
            dataGridViewPacket.Columns["ApprovedDate"].Width = 200;
        ...
        private void btnApproved_Click(object sender, EventArgs e)
        {
            ...
            if ((Helper.AuthorityControl("APPROVE_PACKAGE") || Helper.IsAdmin()) && packetId != 0)
            {
                try
                {
                                            ...
                                            using (SqlConnection conn = new SqlConnection(ConnectionHelper.ConnectionString))
                                            {
                                                CommandText = "UPDATE AYZ_PW_PACKET" +
                                                              ...
                                                              "\nTOTAL_REQUIRED = (SELECT SUM(AMOUNT_REQUIRED) FROM AYZ_PW_PACKET_DETAIL " +
                                                                                  "WHERE PACKETID = '" + packetId + "')" +
                                                              "\n,TOTAL_PAID = (SELECT SUM(AMOUNT_PAID) FROM AYZ_PW_PACKET_DETAIL WHERE PACKETID = '" + packetId + "')" +
                                                              "\nWHERE ID = '" + packetId + "'";
                                                komut.CommandText = CommandText;
                                                komut.Connection = conn;
                                                conn.Open();
