                ...
                using (SqlConnection conn = new SqlConnection(ConnectionHelper.ConnectionString))
                {
                    if (detailSendingValue == 2)
                    {
                        using (SqlCommand cmd = new SqlCommand("AYZ_SP_PW_PAYTRANS", conn))
                        {
                            ...
                            cmd.Parameters.Add("@payBegDate", SqlDbType.DateTime).Value = payBegDate.ToShortDateString();
                            cmd.Parameters.Add("@payEndDate", SqlDbType.DateTime).Value = payEndDate.ToShortDateString();
                            cmd.Parameters.Add("@invBegDate", SqlDbType.DateTime).Value = invBegDate.ToShortDateString();
                            cmd.Parameters.Add("@invEndDate", SqlDbType.DateTime).Value = invEndDate.ToShortDateString();
